...
class reAdamW(Optimizer):
    ...
    def __setstate__(self, state):
        super(reAdamW, self).__setstate__(state)
...
class reSGD(Optimizer):
    def __init__(self, params, lr=required, weight_decay=0, eps=1e-8, betas=(0.9, 0.999)):
        defaults = dict(lr=lr, weight_decay=weight_decay)
        super(reSGD, self).__init__(params, defaults)
        num_params = sum([len(i['params']) for i in params])
        self.lr = lr

    def __setstate__(self, state):
        super(reSGD, self).__setstate__(state)
