...
class GPT2LMModel(nn.Module):
    ...
    def forward(
        ...
        input_ids = input_ids.to(self.transformer.wte.weight.device)
        if lm_labels is not None:
            lm_labels = lm_labels.to(self.transformer.wte.weight.device)
        if lm_mask is not None:
            lm_mask = lm_mask.to(self.transformer.wte.weight.device)
